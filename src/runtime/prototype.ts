...
export function initPrototype(k: BeepContext) {
  ...
  k.defineNamedPrototype = (name: SymbolObj): NamedPrototypeTypeObj => {
    ...
    const defOwnMethod = k.makeDefMethodNative<NamedPrototypeTypeObj>(namedPrototypeType, { binding: 'own' });
...
export function initPrototypeMethods(k: BeepContext) {
  const defMethod = k.makeDefMethodNative<NamedPrototypeTypeObj>(k.prototypeTypeObj);
