...
export function initStruct(k: BeepContext) {
  ...
  k.defineNamedStruct = (name: SymbolObj, fields: SymbolObj[]): NamedStructTypeObj => {
    ...
    const defInstanceMethod = k.makeDefMethodNative<NamedStructObj>(namedStructType);
    ...
    const defOwnMethod = k.makeDefMethodNative<NamedStructTypeObj>(namedStructType, { binding: 'own' });
...
export function initStructMethods(k: BeepContext) {
  const defMethod = k.makeDefMethodNative<NamedStructTypeObj>(k.structTypeObj);
